...
struct hash_table_v1 *hash_table_v1_create() 
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		int err = pthread_mutex_init(&foo_mutex, NULL);
		if (err != 0) {
			exit(err);
		}
	}
...
bool hash_table_v1_contains(struct hash_table_v1 *hash_table,
                            const char *key) // don't modify this function
{
...
void hash_table_v1_add_entry(struct hash_table_v1 *hash_table,
                             ...
                             uint32_t value)
{
	int err = pthread_mutex_lock(&foo_mutex);
	if (err != 0) {
		exit(err);
	}
	struct hash_table_entry *hash_table_entry = get_hash_table_entry(hash_table, key);
...
void hash_table_v1_destroy(struct hash_table_v1 *hash_table)
{
	...
	int err = pthread_mutex_destroy(&foo_mutex);
	if (err != 0) {
		exit(err);
	}
	free(hash_table);
